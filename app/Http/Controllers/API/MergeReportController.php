...
class MergeReportController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $query = Transaction::query()->select(
      ...
      DB::raw("(transaction.total_transfer_quantity - IFNULL((SELECT t3.total_transfer_quantity FROM transaction t3 WHERE t3.bag_id=transaction.bag_id AND t3.id < transaction.id ORDER BY t3.ID DESC LIMIT 1), 0)) as transferred_qty"),
      DB::raw("ROUND((transaction.total_receive_weight - IFNULL((SELECT t3.total_receive_weight FROM transaction t3 WHERE t3.bag_id=transaction.bag_id AND t3.id < transaction.id ORDER BY t3.ID DESC LIMIT 1), 0)), 3) as transferred_weight"),
