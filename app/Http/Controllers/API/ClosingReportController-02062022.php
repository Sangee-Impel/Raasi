...
class ClosingReportController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $query =  "SELECT (ROUND((SUM(b3.pre_casting_inward) + SUM(b3.pre_kambi_inward) + SUM(b3.pre_fancy_inward) + SUM(b3.pre_others_inward)) - (SUM(b3.pre_scrap_outward) + SUM(b3.pre_channam_outward) + SUM(b3.pre_loss) + SUM(b3.bs_closing)),3)) as opening, ";
    ...
    $query .= "IFNULL(sum((SELECT t1.total_receive_weight FROM transaction t1 WHERE t1.bag_id = b.id AND t1.transaction_date < '" . $from_date_raw . "' order by t1.id desc limit 1)), 0) as opening, ";
    $query .= "(SELECT IFNULL(sum(bs1.weight), 0) FROM bag_styles bs1 JOIN bag b1 on bs1.bag_id=b1.id WHERE b1.id=b.id AND bs1.other_accessories_id IS NULL AND b1.created_at < '" . $from_date_raw . "' and b1.id not in (SELECT t1.bag_id FROM transaction t1 WHERE t1.bag_id = b.id AND t1.transaction_date < '" . $from_date_raw . "')) as bs_opening, ";
