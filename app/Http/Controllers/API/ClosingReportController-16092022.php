...
class ClosingReportController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $query .= "IFNULL( ";
    $query .=  "(CASE ";
    $query .=  "WHEN (IFNULL((SELECT count(1) FROM transaction t1 WHERE t1.bag_id in (select distinct bs2.bag_id from bag_styles bs2 JOIN bag b2 on bs2.bag_id=b2.id WHERE b2.id=b.id AND bs2.style_id is not null AND bs2.created_at >= '" . $from_date . "' AND bs2.created_at <= '" . $to_date . "')), 0) > 0) THEN ";
    ...
    $query .= "IFNULL( ";
    $query .=  "(CASE ";
    $query .=  "WHEN (IFNULL((SELECT count(1) FROM transaction t1 WHERE t1.bag_id in (select distinct bs2.bag_id from bag_styles bs2 JOIN bag b2 on bs2.bag_id=b2.id WHERE b2.id=b.id AND bs2.style_id is not null AND bs2.created_at < '" . $from_date_raw . "')), 0) > 0) THEN ";
    ...
    IFNULL(
      (CASE 
      WHEN (IFNULL((SELECT count(1) FROM transaction t1 WHERE t1.bag_id = bs2.bag_id), 0) > 0) THEN 
    ...
    IFNULL(
      (CASE 
      WHEN (IFNULL((SELECT count(1) FROM transaction t1 WHERE t1.bag_id = bs3.bag_id), 0) > 0) THEN 
