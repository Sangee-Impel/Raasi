...
Vue.component('manage-process', {
    ...
    methods : {
        ...
        onAction (action, data, index) {
            ...
            swal({
                    title: "Are you sure?",
                    text: "Process  will be Restored!",
                    icon: "warning",
                     buttons: true,
                     dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.get('/api/process/restore/'+data.id)
                        .then(response => {
                        console.log(response);
            ...
            swal({
                    title: "Are you sure?",
                    text: "Process will be deleted!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.delete('/api/process/'+data.id +"?trash="+this.vueTableParams.trash)
                        .then(response => {
                ...
                .catch(reason => {
                    self.$snotify.error('Cannot delete Process');
