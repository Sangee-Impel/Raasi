...
Vue.component('manage-product', {
    ...
    methods : {
        checkUploadStatus(){
            let uploadResponce = JSON.parse($("#uploadResponce").val());
            if( uploadResponce.status != undefined ){
                if( uploadResponce.status == 1 ){
                    this.$snotify.success(uploadResponce.message);
                }else{
                    this.$snotify.error(uploadResponce.message);
                }
            }
        },
        ...
        onAction (action, data, index) {
            ...
            swal({
                    title: "Are you sure?",
                    text: "Product  will be Restored!",
                    icon: "warning",
                     buttons: true,
                     dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.get('/api/product/restore/'+data.id)
                        .then(response => {
                        console.log(response);
            ...
            swal({
                ...
                if (isConfirm) {
                    axios.delete('/api/product/'+data.id +"?trash="+this.vueTableParams.trash)
                        .then(response => {
                ...
                .catch(reason => {
                    self.$snotify.error('Cannot delete Product');
