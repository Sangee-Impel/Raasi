...
Vue.component('manage-roles', {
    ...
    data() {
        return {
            ...
            trashText       : 'Trash',
            is_advance_search_status:false,
            vueTableParams : {
                trash     : 0,
            }
    ...
    methods : {
        ...
        onAction (action, data, index) {
            ...
            switch (action) {
                ...
                swal({
                                ...
                                .then((willDelete) => {
                                  if (willDelete) {
                                     axios.get('/api/roles/restore/'+data.id)
                                     .then(response => {
                                           console.log(response);
                ...
                .then((willDelete) => {
                  if (willDelete) {
                     axios.delete('/api/roles/'+data.id +"?trash="+this.vueTableParams.trash)
                     .then(response => {
                     ...
                     .catch(reason => {
                                            console.log(reason);
                                        self.$snotify.error('Cannot delete role');
                                        self.isLoading = false;
                                    })
                    } else {
                    this.isLoading = false;
        ...
        reloadtable(all){
            ...
            this.$refs.vuetable.refresh();/*grid reload*/
            // this.isLoading = false;
