...
Vue.component('manage-uom', {
    ...
    methods : {
        ...
        onAction (action, data, index) {
            ...
            swal({
                    title: "Are you sure?",
                    text: "Uom  will be Restored!",
                    icon: "warning",
                     buttons: true,
                     dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.get('/api/uom/restore/'+data.id)
                        .then(response => {
                        console.log(response);
            ...
            swal({
                    title: "Are you sure?",
                    text: "Uom will be deleted!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.delete('/api/uom/'+data.id +"?trash="+this.vueTableParams.trash)
                        .then(response => {
                ...
                .catch(reason => {
                    self.$snotify.error('Cannot delete Uom');
