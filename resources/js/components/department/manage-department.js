...
Vue.component('manage-department', {
    ...
    methods : {
        ...
        onAction (action, data, index) {
            ...
            switch (action) {
                case 'edit-item':
                    axios.get('/api/department/'+data.id)
                        .then(response => {
            ...
            swal({
                    title: "Are you sure?",
                    text: "Department  will be Restored!",
                    icon: "warning",
                     buttons: true,
                     dangerMode: true,
                }).then(isConfirm =>{
                if (isConfirm) {
                    axios.get('/api/department/restore/'+data.id)
                        .then(response => {
                        console.log(response);
            ...
            swal({
                ...
                if (isConfirm) {
                    axios.delete('/api/department/'+data.id +"?trash="+this.vueTableParams.trash)
                        .then(response => {
                ...
                .catch(reason => {
                    self.$snotify.error('Cannot delete Department');
